...
const char *STATUS_TO_STRING[5] = {"Ready", "W_Read", "W_Write", "W_Intr", "W_Time"};

void view_stack_task(int task_index)
{
	int i;
	unsigned int *dump = (unsigned int*)tasks[task_index].stack;
	
	//for(i=0; i!= STACK_SIZE; ++i){
	for(i=0; i!= 32; ++i){
		if( *(dump+i) != 0)
			printf("stack_add %d = %08X\r\n", i, *(dump+i));
	}

	//what happen if task return?
	while(1);
}
...
void _parse_and_process_command(char* command)
{
	...
	*arguments = '\0'; // saperate the command and arguments
	...
	if(!strcmp(command, "echo")){
		...
		for(task_iterator = 0; task_iterator != TASK_LIMIT; ++task_iterator){
			if( tasks[task_iterator].pid == 0 && task_iterator != 0){
				printf("empty task at slot %d\n\r", task_iterator);
			}
			else{
				printf("%3d %10s     %8d\n\r", 
				  tasks[task_iterator].pid, 
				  STATUS_TO_STRING[tasks[task_iterator].status], 
				  tasks[task_iterator].priority);
			}
		}
	}
	else if(!strcmp(command, "view_stack")){
		if (!fork()) 
			setpriority(0, PRIORITY_DEFAULT + 10), view_stack_task(*arguments - '0');
		else
			printf("Create new task, view_stack %d", *arguments - '0');
	}
	else{
